...
# Extract numeric values (remove "PH:")
df_acov['numeric_values'] = df_acov['GMID'].str.extract(r'(\d+)').astype(int)
df_acov['GMID'] = df_acov['numeric_values']
df_acov.drop(columns=['numeric_values'], inplace=True)


df_monthly = pd.read_excel(path+"/monthly_ALL.xlsx")
df_cumulative = pd.read_excel(path+"/cumulative_ALL.xlsx")
